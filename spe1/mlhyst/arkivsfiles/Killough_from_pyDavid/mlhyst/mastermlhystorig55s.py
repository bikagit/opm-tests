...
def trainNN(krnw, Smax):
    ...
    y = np.array(krnw)
    #
    # xconvertedArray = np.array(Smax, dtype=np.float32)
    # yconvertedArray = y.astype(np.float)
    # new_list = list(np.float(Smax))
    # new_list = [float(i) for i in Smax]
    datax1 =np.linspace(0.0, 1.0, 30)
    # datax2 =
    datay= krnw
    ...
    # #
    # # #
    # model.get_layer(model.layers[0].name).adapt(data=datax1)
    # model.get_layer(model.layers[-1].name).adapt(data=datay)

    # Compile the model
    model.compile(optimizer='adam', loss='mean_squared_error')
    # # ft the model on the training dataset
    history = model.fit(X_train, y_train, epochs=350, batch_size=64, validation_split=0.2)
    # make predictions for the input data
    # model.save("models/trainNNhyst.keras")
...
krnw, krw, SandSmax = PreprocessData(SS, "../CO2_KILLOUGH", "GW")
