...
generate_plots() {
    ...
    for cell in $(seq 1 $num_cells); do
        ...
        -labels "${labels}" -ylabel "Saturation at cell ${cell}" \
        -yformat .2f -xlnum 7 -tunits ${tunits} -save=${save_prefix}cell${cell} --output=${output}
    done
}
...
# generate_plots Carlson $FOLDER/Figures/CarlsonFiguresOutput "b,c,k" "ML-Carlson-hysteresis  Carlson hysteresis  No-hysteresis" \
# "$FOLDER/Carlsonmlhystnew-output/CORE_EXAMPLECARLSON $FOLDER/ClassicCarlson-hystnew-output/CORE_EXAMPLECARLSON $FOLDER/Nohysteresis-output/CORE_EXAMPLENOHYST" h curvecarlson 9

# generate_plots LSLBM $FOLDER/Figures/LSLBMFiguresOutput "b,g,r" "ML-Carlson-hysteresis  ML-Killough-hysteresis  ML-LS-LBM" \
# "$FOLDER/Carlsonmlhystnew-output/CORE_EXAMPLECARLSON $FOLDER/Killoughmlhystnew-output/CORE_EXAMPLEKILLOUGH $FOLDER/LSLBMmlhystnew-output/CORE_EXAMPLE" hours curvelslbm 9

# # Combined plot
# for cell in $(seq 1 9); do
#     plopm -i "$FOLDER/Carlsonmlhystnew-output/CORE_EXAMPLECARLSON $FOLDER/Killoughmlhystnew-output/CORE_EXAMPLEKILLOUGH $FOLDER/LSLBMmlhystnew-output/CORE_EXAMPLE $FOLDER/Nohysteresis-output/CORE_EXAMPLENOHYST" \
#     --variable "swat" \
#     -s "${cell},1,1 ${cell},1,1 ${cell},1,1 ${cell},1,1" \
#     -c "b,g,r,k" -lw "1.5,1.5,1.5,1.5" -e "solid,solid,solid,dotted" \
#     -labels "ML-Carlson-hysteresis  ML-Killough-hysteresis  ML-LS-LBM  No-hysteresis" \
#     -ylabel "Saturation at cell ${cell}" -yformat .2f -xlnum 7 -tunits h -save=lslbmcell${cell} --output=$FOLDER/Figures/LSLBMOutputFiguresfull
# done
...
#     #     --legends "Nohyst" "Killough" "Carlson" "LSLBM" \
#     #     --cells "$cell" 1 1 \
