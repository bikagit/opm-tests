...
def trainNN(krnw, Smax):
    ...
    model = create_model(X_train.shape[1],10, [3]*10, Adam(learning_rate=0.05))
...
krnw_modl, krw_modl, SandSmax_modl = Preprocess_analytical_Model_Data(SatSpace, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/SPE1CASE2_2P-1D-OILINJMULTI", "WO")
...
# plt.subplot(1, 6, 4)
x = np.array(SandSmax_modl)
y = np.array(krnw_modl)
...
# plt.scatter(x[:, 0], y_pred_all, color='black', marker='o', label='DNN')
# plt.scatter(x_val[:, 0], y_pred_val, color='red', marker='o', label='DNN')
