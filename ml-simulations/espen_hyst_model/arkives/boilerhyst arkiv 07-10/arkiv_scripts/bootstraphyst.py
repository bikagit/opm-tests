...
def trainNN(krnw, Smax):
    ...
    x_resampled, y_resampled = resample(x, y, n_samples=len(x) * 9, random_state=42)
#
...
#    X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=42)
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True, start_from_epoch=300)
    # Compile the model
    model.compile(optimizer=optimizer, loss='mean_squared_error')
    # Fit the model on the training dataset
    history = model.fit(X_train, y_train, epochs=1350, batch_size=64, validation_data=(X_test, y_test), callbacks=[early_stopping])
