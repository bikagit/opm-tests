...
def trainNN(krnw, Smax):
    ...
    x_resampled, y_resampled = resample(x, y, n_samples=len(x) * 50, random_state=42)
    #
    # X_train, X_test, y_train, y_test = train_test_split(x_resampled, y_resampled, test_size=0.2, random_state=42)
    # Generate synthetic data
    synthetic_x, synthetic_y = generate_synthetic_data(x_resampled, y_resampled, num_samples=len(x) * 50)
...
SSC.append(np.linspace(0.45,0.9,30))
SSC.append(np.linspace(0.30,0.9,30))
SSC.append(np.linspace(0.0618,0.9,30))
...
evaluate(SSC, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/SPE1CASE2_2P", "WO")
...
krnw, krw, SandSmax = PreprocessData(SS, '/data/arkivdata/KrPcData_Combined006.csv', "GW")

orikrnw, orikrw, oriSandSmax = PreprocessData(SS, '/data/arkivdata/KrPcData_Swi0_06-CORRECTED.csv', "GW")
orikrnw1, orikrw1, oriSandSmax1 = PreprocessData(SS, '/data/arkivdata/KrPcData_Swi0_06.csv', "GW")
...
orix1 = np.array(oriSandSmax1)
oriy1 = np.array(orikrnw)

y_pred_all = modelnonwett.predict(x)


for i, (start, end) in enumerate(index_ranges_imbib):
    plt.scatter(orix[start:end, 0], oriy[start:end], alpha=0.6, color='cyan',marker="*", label='Original Imbib' if i == 0 else "")
    plt.scatter(orix1[start:end, 0], oriy1[start:end], alpha=0.6, color='red', label='Original Imbib' if i == 0 else "")
