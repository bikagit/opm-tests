...
# File: killough-data_vs_ML-Carlson_separate_png.py
...
def trainNN(krnw, Smax):
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=50, restore_best_weights=True, start_from_epoch=500)
...
if __name__ == "__main__":
    ...
    newkrnw, newkrw, newSandSmax = newPreprocessData(SSI, newtrain_file,
        "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-A/test006/CORE_ExampleCarlson", "WO")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'test_parity_carlson.png'), dpi=300)
    plt.close()
    ...
    for valsatmax in {0.25, 0.3, 0.35, 0.4, 0.5}:
        ...
        if x_vals and y_vals:
            plt.plot(x_vals, y_vals, color='black', marker='*',
                     label='ML-Carlson Imbib. scan curves' if valsatmax == 0.25 else '')

    maincurves0 = [np.linspace(1.0, 0.0, 30)]
    maincurves1 = [np.linspace(0.0, 1.0, 30)]
    evaluate(maincurves0,
             "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-A/test006/CORE_ExampleCarlson",
             "WO", labelfig="Primary drainage")
    evaluate(maincurves1,
             "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-A/test006/CORE_ExampleCarlson",
             "WO", labelfig="First imbibition")
    ...
    evaluate(SatSpace,
             "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-A/test006/CORE_ExampleCarlson",
             "WO", labelfig="Imbib. Carlson scan curve")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'drainage_imbibition_carlson.png'), dpi=300)
    plt.close()
    ...
    plt.xlabel('Actual'); plt.ylabel('Predicted'); plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'validation_parity_carlson.png'), dpi=300)
    plt.close()
    ...
    print("Saved PNGs:")
    print(" -", os.path.join(output_folder, 'test_parity_carlson.png'))
    print(" -", os.path.join(output_folder, 'drainage_imbibition_carlson.png'))
    print(" -", os.path.join(output_folder, 'validation_parity_carlson.png'))
