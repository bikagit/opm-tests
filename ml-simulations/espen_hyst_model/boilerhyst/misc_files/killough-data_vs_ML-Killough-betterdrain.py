...
def trainNN(krnw, Smax):
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True, start_from_epoch=3000)
...
SSI = []
for smax in S:
    SSI.append(np.linspace(1-smax,1,20))
...
# Plotting lines instead of dots
for valsatmax in {0.01,0.25,0.3,0.35, 0.4, 0.5}:
    # valsatmax= 0.59
    x_vals = []
    y_vals = []
    for valsat in satspace:
