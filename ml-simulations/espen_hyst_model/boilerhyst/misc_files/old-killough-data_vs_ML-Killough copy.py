...
SSI = []
for smax in S:
    SSI.append(np.linspace(1-smax,1,30))
...
# for valsatmax in {0.3,0.35, 0.4, 0.5,1}:
valsatmax= 0.9
x_vals = []
y_vals = []
for valsat in satspace:
...
if x_vals and y_vals:
    plt.scatter(x_vals, y_vals, color="black", marker="*", label='DNN Imbib. scan curves' if valsatmax == 0.3 else "")


# # Plotting lines instead of dots
# for valsatmax in {0.25,0.3,0.35, 0.4, 0.5,0.99}:
#     # valsatmax= 0.59
#     x_vals = []
#     y_vals = []
#     for valsat in satspace:
...
#             y_vals.append(y[0])
#     if x_vals and y_vals:
#         plt.plot(x_vals, y_vals, color="cyan", label="Model prediction" if valsatmax == 0.25 else "")
...
evaluate(SatSpace, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/SPE1CASE2_2P-1D-OILINJMULTI", "WO",labelfig="Imbibition Killough scan curve")
