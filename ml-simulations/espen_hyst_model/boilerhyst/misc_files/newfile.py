...
def evaluate(Sh, data, wp):
    ...
    for S in Sh:
        ...
        if first:
            ...
            plt.plot(1-S, krnw[start:end],color="green")
...
def trainNN(krnw, Smax):
    ...
    x_resampled, y_resampled = resample(x, y, n_samples=len(x) * 35, random_state=42)
    ...
    model = create_model(X_train.shape[1], 10, [30]*10, Adam(learning_rate=0.1))
...
SSI = []
for smax in S:
    SSI.append(np.linspace(1-smax,1,15))
...
satspace = np.linspace(0.0, 0.95, 100)
satMaxspace = np.linspace(0.0, 0.95, 30)
...
#             if first:
#                 plt.plot(valsat, y, marker="o", markersize=3,
...
# # Example input ranges (replace with your actual data)
# satMaxspace = np.linspace(0.2, 1.0, 10)
# satspace = np.linspace(0.1, 0.9, 10)

# # Dummy model for demonstration (replace with your actual model)
# class DummyModel:
#     def predict(self, X):
#         return X[:, 0] * X[:, 1]  # Example: product of valsat and valsatmax

# modelnonwett = DummyModel()
...
for valsatmax in satMaxspace:
    ...
    if x_vals and y_vals:
        plt.plot(x_vals, y_vals, color="red", label="Model prediction" if valsatmax == satMaxspace[0] else "")
