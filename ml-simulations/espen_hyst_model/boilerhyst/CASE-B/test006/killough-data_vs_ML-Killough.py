...
def create_model(input_dim, layers, neurons, optimizer):
    ...
    #     model.add(Dense(neurons[i], activation='hard_sigmoid'))

    # model.add(Dense(1, activation='hard_sigmoid'))  # Output layer for regression
    # model.add(Dense(1, activation='sigmoid'))  # Output layer for regression
    # model.add(Dense(1, activation='hard_sigmoid'))  # Output layer for regression
    # model.add(Dense(16, activation='tanh'))  # Output layer for regression
    model.add(Dense(1, activation='sigmoid'))  # Output layer for regression
...
def trainNN(krnw, Smax):
    ...
    x_resampled, y_resampled = resample(x, y, n_samples=len(x) * 10, random_state=42)
    synthetic_x, synthetic_y = generate_synthetic_data(x_resampled, y_resampled, num_samples=len(x) * 10)
    ...
    model = create_model(X_train.shape[1], 5, [30]*5, Adam(learning_rate=0.005))
    early_stopping = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True, start_from_epoch=100)
    history = model.fit(X_train, y_train, epochs=2000, batch_size=32, validation_data=(x_val, y_val), callbacks=[early_stopping])
...
newtrain_file = '/../../../../../spe1/swat_oilkr2.csv'

krnw, krw, SandSmax = PreprocessData(SSI, train_file, "GW")


newkrnw, newkrw, newSandSmax = newPreprocessData(SSI, newtrain_file,"/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-B/test006/CORE_ExampleKillough", "WO")

original_krnw, original_krw, original_SandSmax = PreprocessData(SSI, '/../../data/curated_data/KrPcData_Swi0_06-CORR-OCT2025.csv', "GW")

krnw_modl, krw_modl, SandSmax_modl = Preprocess_analytical_Model_Data(SSI, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-B/test006/CORE_ExampleKillough", "WO")
...
evaluate(maincurves0, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-B/test006/CORE_ExampleKillough", "WO",labelfig="Primary drainage")
evaluate(maincurves1, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-B/test006/CORE_ExampleKillough", "WO",labelfig="First imbibition")

evaluate(SatSpace, "/Users/macbookn/activopmwkspc/edgedev/opm-tests/spe1/CASE-B/test006/CORE_ExampleKillough", "WO",labelfig="Imbib. Killough scan curve")
