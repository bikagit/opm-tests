#!/bin/bash
# Run all 6 Flow simulations (3 classic + 3 ML) and generate plots
# for both CASE-A/test006 and CASE-B/test006.
set -euo pipefail
...
run_case() {
    local CASE=$1   # e.g. CASE-A/test006
    ...
    # ML simulations
    update_model "$MFOLDER/model/oldmodelkrnwCORE_ExampleKillough.model"
    ...
    for cell in $(seq 1 9); do
        plopm -i "$FOLDER/Killoughmlhystnew-output/CORE_EXAMPLEKILLOUGH $FOLDER/ClassicKillough-hystnew-output/CORE_EXAMPLEKILLOUGH $FOLDER/Nohysteresis-output/CORE_EXAMPLENOHYST" \
            --variable swat -s "${cell},1,1 ${cell},1,1 ${cell},1,1" \
            -c "g,y,k" -lw "1.5,1.5,1.5" -e "solid,dashed,dotted" \
            -labels "ML-Killough-hyst  Killough hyst  No-hyst" \
            -ylabel "Saturation (Sw) at cell ${cell}" -yformat .2f -xlnum 10 -tunits d \
            -save=curvekilloughcell${cell} --output="$FOLDER/Figures/KilloughFiguresOutput"
        ...
        plopm -i "$FOLDER/Carlsonmlhystnew-output/CORE_EXAMPLECARLSON $FOLDER/Killoughmlhystnew-output/CORE_EXAMPLEKILLOUGH $FOLDER/LSLBMmlhystnew-output/CORE_EXAMPLE $FOLDER/Nohysteresis-output/CORE_EXAMPLENOHYST" \
            --variable swat -s "${cell},1,1 ${cell},1,1 ${cell},1,1 ${cell},1,1" \
...
run_case "CASE-A/test006"
run_case "CASE-B/test006"
echo ""
echo "[INFO] ALL 006 CASES COMPLETE"
